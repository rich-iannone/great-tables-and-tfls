...
Four columns will receive number formatting, this time to 3 significant figures.
...
Let's remove all of those cells that have `NA` in them. We can do this with the `sub_missing()` method.
...
Writing the table to HTML can be done with the `save()` method.

```{python}
gt_table_final.save("html_14.2.01.html")
```
